...
| Closed | 2026-05-18 by @po (Pax) — PASS 95/100, CI green |
...
| QA gate | PASS 95/100 (`docs/gates/STORY-001.2-gate.md`) |
| CI | green — GitHub Actions run #26012342162 (3 jobs) |
| Commit head | `26bb711` on `origin/main` (8 atomic commits ahead of pre-story baseline) |
| G4 contract | preserved verbatim across all `runner.py` edits (test pair passes) |

**CHK adaptations (cross-repo, deploy_type=none):**
- CHK-8 (deploy verification): N/A — `deploy_type: none` (Python library/CLI). CI green on `origin/main` is the equivalent verification surface.
- CHK-9 (registry governance): N/A — `claude-i` has no AIOX registry surface (no `services/`, no `squads/`, no `ecosystem-registry.yaml`).
- CHK-10 (IDS post-check): N/A — no services / squads / skills were touched (all changes scoped to `src/claude_i/*` + `tests/*` + `pyproject.toml`).

**Forward-compat carryovers → STORY-001.5:**
- `src/claude_i/exit_codes.py` is the source of truth for `doctor` / `uninstall` / `reap` subcommands. Reuse the constants (`SUCCESS`, `RUNTIME_ERROR`, `CONFIG_ERROR`, `PLATFORM_ERROR`); do not redefine.
- G14 (SubagentStop event discovery) — deferred to 001.5.
- G17 (readiness polling, replacing fixed `--ready-wait` sleep) — deferred to 001.5.

**QA non-blocking issues filed for backlog (from gate Recommendations / Future):**
1. `NOTES.md` does not yet document G14 + G17 carryovers (currently only in this story's Dev Agent Record). Convention from 001.1 is that deferrals land in NOTES.md for post-merge discoverability. Suggested owner: @dev, pick up at the start of 001.5.
2. `src/claude_i/reaper.py:74` retains a bare `sys.exit(1)` in `_sigterm_handler` instead of `sys.exit(RUNTIME_ERROR)`. Functionally identical (1 == RUNTIME_ERROR) — cosmetic consistency only. Suggested owner: @dev, migrate during 001.5 when re-touching `reaper.py` for `reap` subcommand.

**Recommended next:** STORY-001.3 (PyPI packaging: build, publish via OIDC, `pipx` + `uv tool` validation, `--version`). Dependencies met (001.0 ✓, 001.1 ✓, 001.2 ✓). Ready for @sm refinement → @po validation → @dev execution.
...
| 2026-05-18 | @po (Pax) | **Closed → Done.** QA gate PASS 95/100. CI green (GitHub Actions run #26012342162, 3 jobs). HEAD `26bb711` on `origin/main`. CHK-8/9/10 all N/A for this repo (cross-repo, `deploy_type: none`, no AIOX governance surface in claude-i). G4 contract preserved verbatim — both layers + test pair intact. Forward-compat carryovers documented in Closure section: G14 + G17 → STORY-001.5; `exit_codes` module is the source of truth. QA non-blocking observations filed for backlog: (1) NOTES.md G14/G17 carryover documentation, (2) `reaper.py:74` bare `sys.exit(1)` → migrate to `RUNTIME_ERROR` constant. Next: STORY-001.3 (PyPI packaging) — dependencies met. |
